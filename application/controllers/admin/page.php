...
/*		$count = count($this->data['all_data']);
    ...
    public function edit($id = NULL)
	{
		...
		$this->db->order_by('order','ase');
		$files = $this->comman_model->get('page_files',false);
		
        foreach($files as $key=>$file){
            $file->thumbnail_url = base_url('assets/templates/images/icons/filetype/_blank.png');
            $file->zoom_enabled = false;
            $file->download_url = base_url($this->data['_cancel'].'/'.$file->filename);
            $file->delete_url = site_url($this->data['_cancel'].'/upload/rep_'.$file->page_id).'?file='.rawurlencode($file->filename);
            ...
            else if(file_exists(FCPATH.'assets/templates/images/icons/filetype/'.get_file_extension($file->filename).'.png'))
            {
                $file->thumbnail_url = base_url('assets/templates/images/icons/filetype/'.get_file_extension($file->filename).'.png');
            }
            $this->data['files'][$file->page_id][] = $file;
        }
        ...
        if($this->form_validation->run() == TRUE){
			$data = $this->pages_model->array_from_post(array('template','slug','top_menu','middle_menu','bottom_menu'));
