...
class Order_history extends Admin_Controller {
	...
	function send_mail($order_id){
		...
		if($order_data){
			...
			$data['address1'] = $user_address->address;
			...
			$data['order_date'] = date('Y-m-d',$user_address->created);
			...
			if($order_items){
				...
				foreach($order_items as $set_value){
					...
					$total_all = ($set_value->price*$set_value->quantity)+$shipping_cost+$total_all;
					$total = ($set_value->price*$set_value->quantity)+$shipping_cost;
					$product = $this->comman_model->get_lang('products',$this->data['content_language_id'],NULL,array('id'=>$set_value->product_id),'product_id',true);
					if($product){
					...
					$attributesString ='';
					$html.='<tr style="text-align:center" class="item-row" ><td>'.$productName.'</td><td>'.$storeName.'</td><td>$'.$set_value->price.'</td><td>'.$set_value->quantity.'</td><td>$'.$total.'</td></tr>';
					
					//check coupon code
					$checkCouponData = $this->comman_model->get_by('user_order_item_coupons',array('order_id'=>$order_data->id,'item_id'=>$set_value->id),false,false,false);
					if($checkCouponData){
						foreach($checkCouponData as $setCheckCoupon){
							$couponCodeHtml .='<tr><td>'.$productName.'</td><td>'.$setCheckCoupon->coupon_code.'</td>';
						}
					}
				}
			...
			$data['total_amount'] = '$'.$total_all;
			$data['total'] = '$'.($total_all+$totalShipping-$order_data->coupon_cost);
			$data['coupon'] = '- $'.$order_data->coupon_cost;
		...
		$email_data->message = str_replace('{address1}', $data['address1'], $email_data->message);
