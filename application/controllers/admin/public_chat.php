...
class Public_chat extends Admin_Controller{
    ...
    public function index()
	{
		...
		$this->load->library('public_chats');
	}
	...
	function get_all(){
		...
		foreach($all_unread as $set_data){
			...
			$user_data = $this->comman_model->get_by('user_online',array('id'=>$set_data->to,'user_type !='=>'admin'),false,false,true);
			if($user_data){
		...
		foreach($all_unread as $set_data){
			...
			$user_data = $this->comman_model->get_by('user_online',array('id'=>$set_data->from,'user_type !='=>'admin'),false,false,true);
			if($user_data){
				$image = 'assets/uploads/profile.jpg';
				$clickUser = "chatWith('".$user_data->id."','".$user_data->user_name."')";
