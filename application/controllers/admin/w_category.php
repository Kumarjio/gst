...
class w_category extends Admin_Controller
{
	public $_table_names = 'w_categories';
	public $_subView = 'admin/w_categories/';
	public $_redirect = '/w_category';
	public function __construct(){
		parent::__construct();
		$this->data['active'] = 'Work Flow Management';
        $this->load->model('w_category_model');
    ...
    public function index()
	{
		...
		$this->data['all_data'] = $this->w_category_model->get_lang();
/*		$count = count($this->data['all_data']);
    ...
    public function order_ajax(){
        ...
        if (isset($_POST['sortable'])) {
            $this->w_category_model->save_order($_POST['sortable']);
        }
        
        // Fetch all pages
        $this->data['pages'] = $this->w_category_model->get_nested($this->data['content_language_id']);
    ...
    public function edit($id = NULL)
	{
	    ...
	    if($id)
        {
            $this->data['categories'] = $this->w_category_model->get_lang($id, FALSE, $this->data['content_language_id']);
        ...
        else
        {
			...
			$this->data['title'] = $this->data['name'].' | '.$this->data['settings']['site_name'];
            $this->data['categories'] = $this->w_category_model->get_new();
        }
        

       
		// Pages for dropdown
        $this->data['pages_no_parents'] = $this->w_category_model->get_no_parents($this->data['content_language_id']);
        $this->data['page_languages'] = $this->language_model->get_form_dropdown('language');
        $this->data['templates_page'] = $this->w_category_model->get_templates('page_');
        // Fetch all files by repository_id
        
        // Set up the form
		$this->form_validation->set_message('required', '%s '.show_static_text($this->data['adminLangSession']['lang_id'],219));

        $rules = $this->w_category_model->rules;
        $this->form_validation->set_rules($this->w_category_model->get_all_rules());

        // Process the form
        if($this->form_validation->run() == TRUE)
        {
            $data =array();
            $data = $this->w_category_model->array_from_post(array('slug','parent_id'));
            if($id == NULL)$data['order'] = $this->w_category_model->max_order()+1;
            $data_lang = $this->w_category_model->array_from_post($this->w_category_model->get_lang_post_fields());
            if($id == NULL){
            ...
            $id = $this->w_category_model->save_with_lang($data, $data_lang, $id);
...
*/
    ...
    public function delete($id){
		$this->w_category_model->delete($id);
    ...
    public function _unique_slug($str)
    {
        ...
        $categories = $this->w_category_model->get();
