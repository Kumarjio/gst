...
class Country extends Admin_Controller
{
    ...
    public function edit($id = NULL){
        ...
        else
        {
			...
			$this->data['title'] = $this->data['name'].' | '.$this->data['settings']['site_name'];
            $this->data['products'] = $this->country_model->get_new();
        }
        ...
        $this->form_validation->set_rules($this->country_model->get_all_rules());
        ...
        if($this->form_validation->run() == TRUE)
        {
            ...
            //if($id == NULL)$data['order'] = $this->country_model->max_order()+1;
//            $data_lang = $this->country_model->array_from_post($this->country_model->get_lang_post_fields());
            if($id == NULL){
                $data['created'] = time();
                $data['modified'] = time();
			}
...
*/
    ...
    public function order_ajax(){
        ...
        if (isset($_POST['sortable'])) {
            $this->country_model->save_order($_POST['sortable']);
        }
        
        // Fetch all pages
        //$this->data['pages'] = $this->country_model->get_nested($this->data['content_language_id']);
		$this->data['pages'] = $this->country_model->get_nested($this->data['content_language_id']);
