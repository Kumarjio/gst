...
//		$id = 'day';
	...
	function set_rating(){
		...
		if(isset($user_data['user_session'])){
			if($user_data['user_session']['loginType']=='user'){
				$check =$this->comman_model->get_by('stores_rating',array('user_id'=>$user_data['user_session']['id'],'store_id'=>$id),false,false,false);
				if(count($check)==0){
					$this->comman_model->save('stores_rating',array('user_id'=>$user_data['user_session']['id'],'store_id'=>$id,'rate'=>$rate));
					$msg = 1;
				}
				else{
				}
	...
	function set_review(){
		...
		$rate = $this->input->post('rate');
		if(isset($this->data['user_details'])){
			$check_review =  $this->comman_model->get_by('stores_rating',array('user_id'=>$this->data['user_details']->id,'store_id'=>$id),FALSE,FALSE,TRUE);	
			if($check_review){
				$msge['msg'] = '<font style="color:#F00">Your already sent your review!!</font>';
			}
			else{
				$this->comman_model->save('stores_rating',array('user_id'=>$this->data['user_details']->id,'username'=>$this->data['user_details']->first_name.' '.$this->data['user_details']->last_name,'store_id'=>$id,'comment'=>$review,'rate'=>$rate));
				$msge['status'] = 'ok';
