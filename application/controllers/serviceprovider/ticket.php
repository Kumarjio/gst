...
class Ticket extends Frontend_Controller{	
	public $_redirect = '/serviceprovider/ticket';

	public $_table_names = 'tickets';

	public $_subView = 'serviceprovider/tickets/';
...
//		$this->checkPermissions('Ticket');
	...
	function index(){
        ...
        $this->data['name'] = show_static_text($this->data['lang_id'],102);
...
//			$this->send_mail($id);
			...
			if($more_pic){
				...
				$this->session->set_flashdata('success_id',295);
...
function ajax_get_hour(){	
	...
	function send_mail($id	= false){
		...
		$checkUser = $this->comman_model->get_by('users',array('id'=>$this->data['user_details']->id),false,false,true);
		if(!$checkUser){
			return false;
		}
		$html = '<h3>Dear admin,</h3><p>'.$checkUser->username.' has created ticket <a href="'.base_url($this->data['settings']['admin_link'].'/ticket').'">Click Here</a>.</p>';
		...
		$this->email->subject('Create Ticket');
