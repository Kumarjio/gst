...
/*					if($set_msge->to_id==$this->data['user_account']->id){
					...
					$temp[] = array(
								...
								"avatar"=>$image,
								"body"=>parse_smileys($set_msge->message, base_url().'assets/plugins/user_chat/smileys/'),
								"time"=>$set_msge->sent,
								"type"=>$type,
								"name"=>$name
								);
...
/*		$to_user = 0;
		...
		if($to_user==0&&$message){
			if(isset($this->data['user_account'])){
				...
				$message = parse_smileys($message, base_url().'assets/plugins/chat/images/smileys/');
