...
class Step_model extends MY_Model {    
	...
	public $s_rules1 = array(
        'name' => array('field'=>'name', 'label'=>'Name', 'rules'=>'trim|reuqired'),
        'description' => array('field'=>'description', 'label'=>'Description', 'rules'=>'trim'),
   );

	public $s_rules3 = array(
        'name' => array('field'=>'name', 'label'=>'Name', 'rules'=>'trim|reuqired'),
        'description' => array('field'=>'description', 'label'=>'Description', 'rules'=>'trim'),
   );

	public $s_rules4 = array(
        'name' => array('field'=>'name', 'label'=>'Name', 'rules'=>'trim|reuqired'),
        'description' => array('field'=>'description', 'label'=>'Description', 'rules'=>'trim'),
   );
	public $s_rules2 = array(
        'name' => array('field'=>'name', 'label'=>'Name', 'rules'=>'trim|reuqired'),
   );
   ...
   public $rules_lang = array();
    ...
    public function get_new_desired(){
        $tags = new stdClass();
        //$tags->parent_id = 0;
        $tags->name 			= '';
        $tags->description	 	= '';
...
/*		$this->db->delete('product_tag', array('product_id'=>$id));
    ...
    public function save_options($data,$options_id, $id){
		if($options_id){
			$this->db->where_not_in('id',$options_id);
			$this->db->where('hair_id',$id);
			$this->db->delete('hairs_quest_option');
		}
		else{
			$this->db->where('hair_id',$id);
			$this->db->delete('hairs_quest_option');
		...
		if($data){
			foreach($data as $key=>$value){
				if(isset($options_id[$key])){
					...
					$this->db->where('hair_id', $id);
					$this->db->update('hairs_quest_option',array('name'=>$value));
				}
				else{
					$this->db->insert('hairs_quest_option',array('name'=>$value,'hair_id'=>$id));
				}
			}
		}
	}
		...
		if($data){
			...
			if($arrData){
				foreach($arrData as $key=>$value){
					$temp = array();
					$questId= trim($key,'q');
					$checkQues = $this->comman_model->get_by('hairs_quest',array('id'=>$questId),false,false,true);
					if($checkQues){
						$temp['quest']= $checkQues->name;
						$temp['ans']= 'None';
						if($value!=0){
							$checAns = $this->comman_model->get_by('hairs_quest_option',array('hair_id'=>$questId,'id'=>$value),false,false,true);
							if($checAns){
								$temp['ans']= $checAns->name;
							}
						}
					}
					$result_arr[] = $temp;
