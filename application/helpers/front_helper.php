<?php
function h_unitPrice($price,$unit,$currency,$show=false){
	if($show){
...
function h_checkdate($x){
    return (date('Y-m-d H:i:s', strtotime($x)) == $x);
}

function skyscanner_api($url){
    $options = array(
...
function h_array_search_inner ($array, $attr, $val, $strict = FALSE) {
  ...
  foreach ($array as $key => $inner) {
    ...
    if ($strict) {
      // Strict typing
      if ($inner[$attr] === $val) return $inner;
    } else {
      // Loose typing
      if ($inner[$attr] == $val) return $inner;
    }
