...
class ci_chat{
	...
	*/
	...
	function sendChat() {
		...
		$messagesan = parse_smileys($messagesan, base_url().'assets/plugins/new_chat/img/smileys/');	
	// parse_smileys($message, base_url().'assets/plugins/new_chat/img/smileys/');	
	...
	EOD;*/
		...
		$this->ci->db->insert('users_chat', $data); 
		echo "1";
		exit(0);
	}
	...
	function chatHistory(){
		...
		$query = $this->ci->db->get('users_chat');
		$items = '';
		$chatBoxes = array();
		foreach ($query->result_array() as $chat) {
			# code...
			$chat['message'] = $this->sanitize($chat['message']);
			$chat['message'] = parse_smileys($chat['message'], base_url().'assets/plugins/new_chat/img/smileys/');	
			$d = array();
			$d['s'] = '0';
			$d['f'] = "$chat[from]";
	...
	?>
{
	...
	EOD;*/
		...
		$sql = "update users_chat set recd = 1 where users_chat.to = '".mysql_real_escape_string($_SESSION['username'])."' and recd = 0";
