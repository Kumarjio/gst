...
if(isset($my_orders)&&!empty($my_orders)){
	foreach($my_orders as $set_order){
		$order_data = $this->comman_model->get_by('user_orders',array('id'=>$set_order->order_id,'payment'=>1,'payment_type'=>'cash'),false,false,true);
		if($order_data){
...
?>
                  <tr id="tablerow_<?=$set_order->id?>">
                    ...
                    <td class="">&pound;<?=numberFormat($set_order->price);?></td>
