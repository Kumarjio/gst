    ...
    <div class="table-responsive">
        <table id="data-table" class="table table-striped table-bordered">
                ...
                <tr>
                    ...
                    <th><?=show_static_text($adminLangSession['lang_id'],258);?>Show</th>
...
if(count($all_data)){
	foreach($all_data as $set_data){
		if(isset($set_data->image)){
			$image = base_url('assets/uploads/countries/small').'/'.$set_data->image; 
		}
		else{
...
?>
                        <tr>
							...
							<td><img src="<?=$image?>" class="img-rounded" style="width:30px;height:30px"> <?php echo $set_data->name; ?></td>
                            <td>
...
function get_active(type,id,value){
	//alert(name+' '+id+' '+value);
    $.ajax({
       type: "POST",
