...
if(count($all_data)){
	foreach($all_data as $set_data){
		...
		if($set_data->store_id!=0){
			$storeData = $this->comman_model->get_by('users',array('id'=>$set_data->store_id),false,false,true);
			if($storeData){
				$storeName = ''.$storeData->username.'';
			}
		}
		...
		$categoryName = $set_data->type;
?>
                        <tr>
							<td><?php echo $set_data->id; ?></td>
							<td><img src="<?=$image?>" class="img-rounded" style="width:30px;height:30px"> <?=$set_data->title; ?></td>
							<td> <?=print_lang_value('categories',$content_language_id,array('id'=>$set_data->category_id),'category_id','title');?></td>
