...
<div class="row">
	    <div class="col-md-6">
    		<div class="btn-group">
			    <a href="<?=$_create?>" class="btn btn-primary m-r-5 m-b-5"><?=show_static_text($lang_id,233);?> <i class="fa fa-plus"></i></a>
		    </div>
	    </div>    	
	    </div>
...
if(count($all_data)){
	foreach($all_data as $set_data){
	   $user = $this->comman_model->get_by('users',array('id'=>$set_data->user_id),false,false,true);
		if(!empty($user)){
			$user_name = $user->first_name.' '.$user->last_name.'<br>('.h_dateFormat($user->dob,'d-m-Y').')';
		}
		else{
			$user_name = 'No User';
		}
?>
                        <tr>
							...
							<td>
                            <a class="btn btn-icon-only btn-info " href="<?=$_cancel.'/send_mail/'.$set_data->id;?>/true" >
										<i class="fa fa-mail-forward"></i></a>
                            <a class="btn btn-icon-only btn-success" href="<?=$_edit.'/'.$set_data->id;?>" >
										<i class="fa fa-edit"></i></a>
                            	<a class="btn btn-icon-only btn-danger " href="<?=$_delete.'/birth/'.$set_data->id;?>"  onclick="return confirm_box();" >
