...
function createChatBox(chatboxtitle,chatuserName, minimizeChatBox) {
	...
	//call
	jq.ajax({
	  url: "admin/public_chat?action=chathistory",
	  cache: false,
...
function chatHeartbeat(){
	...
	jq.ajax({
	  url: "admin/public_chat?action=chatheartbeat",
	  cache: false,
	  dataType: "json",
	  success: function(data) {
...
function closeChatBox(chatboxtitle) {
	...
	jq.post("admin/public_chat?action=closechat", { chatbox: chatboxtitle,csrf_test_name:csrfTokenValue} , function(data){	
	});
...
function checkChatBoxInputKey(event,chatboxtextarea,chatboxtitle, boxUserName) {
	...
	if(event.keyCode == 13 && event.shiftKey == 0)  {
		...
		if (message != '') {
			jq.post("admin/public_chat?action=sendchat", {to: chatboxtitle, to_name: boxUserName, message: message,csrf_test_name:csrfTokenValue} , function(data){
...
function startChatSession(){  
	jq.ajax({
	  url: "admin/public_chat?action=startchatsession",
	  cache: false,
	  dataType: "json",
	  success: function(data) {
